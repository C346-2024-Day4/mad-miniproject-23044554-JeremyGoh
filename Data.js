export const datasource = [
    {
        title: "Breakfast Recipes",
        data: [
            {
                key: 'Pancakes',
                image:'https://food-images.files.bbci.co.uk/food/recipes/fluffyamericanpancak_74828_16x9.jpg',
                ingredients: 'Flour, eggs, milk, sugar, butter, syrup, berries',
                cookingTime: 15,
            },
            {
                key: 'Omelette',
                image: 'https://www.thespruceeats.com/thmb/jE-PesfSjhsGS3CFBXIQNu_1tqo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/dinner-omelet-480277-hero-01-84a5af20b08440598a2b5b662f2e5cbe.jpg',
                ingredients: 'Eggs, cheese, vegetables, salt, pepper',
                cookingTime: 10,
            },
            {
                key: 'Smoothie Bowl',
                image: 'https://minimalistbaker.com/wp-content/uploads/2016/07/The-PERFECT-5-minute-Smoothie-Bowl-Simple-ingredients-naturally-sweet-SO-healthy-vegan-glutenfree-smoothiebowl-recipe.jpg',
                ingredients: 'Bananas, berries, yogurt, granola, honey',
                cookingTime: 5,
            },
        ],
        bgcolor: "#789DBC",
    },
    {
        title: "Lunch Recipes",
        data: [
            {
                key: 'Grilled Cheese Sandwich',
                image: 'https://cdn.loveandlemons.com/wp-content/uploads/2023/01/grilled-cheese.jpg',
                ingredients: 'Bread, cheese, butter',
                cookingTime: 10,
            },
            {
                key: 'Caesar Salad',
                image: 'https://www.seriouseats.com/thmb/Fi_FEyVa3_-_uzfXh6OdLrzal2M=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/the-best-caesar-salad-recipe-06-40e70f549ba2489db09355abd62f79a9.jpg',
                ingredients: 'Romaine lettuce, croutons, Caesar dressing, Parmesan cheese',
                cookingTime: 15,
            },
            {
                key: 'Prawn Aglio Olio',
                image: 'https://simply-delicious-food.com/wp-content/uploads/2019/09/Shrimp-Aglio-Olio-4.jpg',
                ingredients: 'Spaghetti, prawn, garlic, olive oil',
                cookingTime: 30,
            },
        ],
        bgcolor: "#D1E7DD",
    },
    {
        title: "Dinner Recipes",
        data: [
            {
                key: 'Ribeye Steak',
                image: 'https://www.wholesomeyum.com/wp-content/uploads/2024/01/wholesomeyum-Ribeye-Steak-Recipe-6.jpg',
                ingredients: 'Ribeye Steak, Butter, Garlic, Herbs, Salt and Pepper',
                cookingTime: 20,
            },
            {
                key: 'Lemon Herb Salmon',
                image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWG7j1z35ZgvN6Jb8iKrP01b5jgovbTgFnwA&s',
                ingredients: 'Salmon fillets, lemon, garlic, olive oil, herbs, salt and pepper',
                cookingTime: 40,
            },
            {
                key: 'Vegetable Stir Fry',
                image: 'https://www.allrecipes.com/thmb/MF7yU1MBbRlaT40ogVr-1PgggKc=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/222658-frozen-vegetable-stir-fry-4x3-1382-583b53fa0bcd4247920611ad431c14cb.jpg',
                ingredients: 'Broccoli, bell peppers, carrots, soy sauce, garlic, ginger',
                cookingTime: 30,
            },
        ],
        bgcolor: "#1F509A",
    },
];
